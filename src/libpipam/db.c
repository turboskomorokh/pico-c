...
int db_exists_pkgname(const char* name)
{
  ...
  if (!name) {
    pico_log_die(LOG_ERROR, "%s(): name is NULL", __func__);
  }
...
{
  if (!pkg->name) {
    pico_log_die(LOG_ERROR, "%s(): pkg->name is NULL", __func__);
  }
...
int db_remove_pkgname(const char* name)
{
  ...
  if (!name) {
    pico_log_die(LOG_ERROR, "%s(): package name is NULL", __func__);
  }
...
int db_check_pkgs_conflict(vec_t* conflicts_list, const char* name)
{
  struct dirent* entry;
  ...
  if (!conflicts_list) {
    pico_log_die(LOG_ERROR, "%s(): conflicts_list is NULL", __func__);
  }
...
#if DEBUG == 1
    pico_log(LOG_DEBUG, "Checking if %s conflicts with %s", installed_pkg->name, name);
#endif
...
#if DEBUG == 1
        pico_log(LOG_DEBUG, "%s conflicts with %s", installed_pkg->name, name);
#endif
        vec_push_back(conflicts_list, installed_pkg->name);
      }
    }
  ...
  closedir(db_dir);
  return (conflicts_list)->len ? EXIT_FAILURE : EXIT_SUCCESS;
}
...
int db_check_pkgs_dependant(vec_t* depends_list, const char* name)
{
  ...
  if (!depends_list) {
    pico_log_die(LOG_ERROR, "%s(): depends_list is NULL", __func__);
  }
